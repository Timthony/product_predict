...
# 将天气的文件处理成csv格式，也就是把天气文件融合起来，最后尺寸5113×2160
...
def main(data_dir='./data/', N=10, cv_test_size=0.3, files_to_use='all', submit_name='submission.csv'):
    ...
    # 将天气文件转为.csv格式
    np.savetxt("zth_train_X.csv", trainX, delimiter=',')
    np.savetxt("zth_train_Y.csv", trainY, delimiter=',')
    ...
    np.savetxt("zth_test_X.csv", testX, delimiter=',')
